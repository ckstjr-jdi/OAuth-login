...
const LoginView = () => {
    ...
    const loginG = async() => {
        ...
        //클라이언트가 구글 서버에 요청(인가코드)을 하고 구글은 듣고 응답으로 보내줄 URL미리 등록해 둠
        //구글서버에게 요청을 하게 되면 응답페이지 처리에 대한 제어권이 구글에게 넘어감
        //그래서 미리 응답을 받을 수 있도록 redirecturl을 통해서 쿼리스트링으로
        //구글이 브라우저에게 인가코드를 넘겨준다
        //인가가코드를 받아서(5173번) 스프링 부트 서버(8000번)에 요청을 전달함.
    ...
    const loginK = async () => {
        ...
        try{
            ...
            console.log("카카오 인증코드 가져오기 실패", error)
        }
    };
